...
#!/usr/bin/env python3
"""
Standalone script for training and evaluating Gaussian Naive Bayes on fetal_health.csv
...
def main():
    ...
    print(f"MCC: {mcc:.4f}")
    with open('gaussian_nb_classification_report.txt', 'w') as f:
        f.write(classification_report(y_test, y_pred))
    cm = confusion_matrix(y_test, y_pred)
    pd.DataFrame(cm).to_csv('gaussian_nb_confusion_matrix.csv', index=False)
